...
with cte as (Select cuisine , Restaurant_id, count(*) as no_of_orders from orders group by cuisine , restaurant_id)
Select * from (
Select * , Row_number()over(partition by cuisine order by no_of_orders desc ) as rn  from cte) a 
where rn < 3;
-- Find daily new customer count from the launch date (everyday how many new customers are we acquiring )
Select customer_code ,min(Placed_at) from orders group by customer_code ;

-- count all the users  who are accquired  in jan 2025  and only placed one order in jan and did not place any other orderin jan and did not place any order alter
...
AND customer_code NOT IN (
      ...
      FROM orders 
      WHERE NOT (EXTRACT(MONTH FROM Placed_At) = 1 AND EXTRACT(YEAR FROM Placed_At) = 2025))
GROUP BY customer_code
having count(*)=1;

-- list all the customer  with no orders  in the last  7 days  but we acquire one month ago with their first order on promo ...
    
    with cte As(SELECT customer_code ,MAX(Placed_At)as Latest_order, Min(Placed_At) as First_order
    FROM orders
    GROUP BY customer_code ) 
    Select cte.* , orders.Promo_code_Name from cte Inner Join orders on cte.customer_code = orders.customer_code and cte.First_order = orders.Placed_At
   where  Latest_order < DATE_SUB(CURDATE(), INTERVAL 7 DAY) and First_order < DATE_SUB(CURDATE(), INTERVAL 1 Month) And orders.Promo_code_Name is not null
 ;
